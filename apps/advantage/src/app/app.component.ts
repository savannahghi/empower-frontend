...
export class AppComponent implements OnInit {
    ...
    constructor(
        ...
        // Session recording is opt-in: without a configured project id the
        // deployment must not call out to Clarity at all.
        const clarityProjectId = environment.empowerClarityProjectId;
        if (
    ...
    private loadGoogleMapsScript(): void {
        // Without a key the request only fails; do not call out at all.
    ...
    private loadJiraServiceDeskWidget(): void {
        // The support widget belongs to whoever runs the deployment.
        if (!environment.jsdKey) return;
