...
export class SilTimeAgoPipe implements PipeTransform {
    ...
    public transform(inputDate: any): string {
        const jsDateObj = moment(inputDate);

        const past = +jsDateObj.toDate();
        const now = +new Date();
        ...
        for (
            let i = 0,
                l = SilTimeAgoPipe.MAPPER.length,
                ms = now - past,
                div = SilTimeAgoPipe.YEAR_MS;
            i < l;
            ++i
        ) {
            const elm = SilTimeAgoPipe.MAPPER[i];
