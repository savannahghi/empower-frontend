 ...
 */
export class OclProductFieldsService {
    /**
        ...
        return [
            {
                ...
                props: {
                    ...
                    bindValue: 'value',
                    options: [
                        { title: 'Drug and Device', value: 'KNC4Drugs' },
                        { title: 'Service', value: 'KNC4Investigations' },
                    ],
                    searchable: false,
                    closeOnSelect: true,
                    required: true,
                ...
                expressions: {
                    'model.source': field => {
                        this.source = field?.model?.source;
                        return field?.model?.source;
                    },
                },
                modelOptions: {
                    updateOn: 'blur',
    ...
    productsResponseFunction = resp => {
        function selectFewerFields(select) {
            ...
            const { id, display_name, concept_class } = select;
            return { id, display_name, concept_class };
        }
        const newArr = resp['data'].map(selectFewerFields);
        this.selectedProduct = newArr;
        return newArr;
    };
