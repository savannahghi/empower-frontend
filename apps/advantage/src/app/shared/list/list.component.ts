...
export class ListComponent implements OnInit, OnDestroy {
    ...
    createdRecord = (data?: any) => {
        this.siltable?.getData();
        this.loading['createRecord'] = false;
        this.toggleModal('createRecord');
        const title = `Add ${this.storeLabel}`;
        // A credential the caller can only be told once, such as a new user's
        // password, is shown rather than dropped.
        const context = data?.temporary_password
            ? `${this.storeLabel} added. Temporary password: ${data.temporary_password}`
            : `${this.storeLabel} added successfully`;
