 ...
 */
@Component({
    selector: 'ngx-breast-cancer-screening',
    templateUrl: './breast-cancer-screening.component.html',
    styleUrls: ['./breast-cancer-screening.component.scss'],
    standalone: false,
})
/**
 * Class that renders the Breast Cancer Screening Component
 * handles the stepper component that guides users through the screening process
 */
export class BreastCancerScreeningComponent implements OnInit, AfterViewInit {
    /**
    ...
    /**
     * Functions to move to the summary page fromthe questionnaire screen
     */
    next() {
        this.loading = true;
    ...
    submitLForm(payload) {
        ...
        this.dataLayer
            .create('questionnaire-response', formData, {
                questionnaireID: questionnaireID,
                encounterID: encounterID,
            })
            .subscribe({
                next: response => {
                    this.analytics.logEvent('screening_breast_created');
    ...
    responseFunction(response) {
        if (response && response.questionnaireResponseID) {
            ...
            this.analytics.logEvent('screening_breast_response_created');
    ...
    /**
     * Function used to update stepper icons and style
     */
    ...
    fetchQuestionnaires() {
        ...
        const param = {
            searchParam: 'Breast Cancer Screening',
        };
