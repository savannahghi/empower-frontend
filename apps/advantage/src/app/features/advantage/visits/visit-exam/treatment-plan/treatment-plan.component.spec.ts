...
describe('TreatmentPlanComponent', () => {
    ...
    it('should test component functions', () => {
        ...
        component.toggleModal('add-diagnosis-modal');
    ...
    it('should test fetchDiagnoses method', () => {
        spyOn(component, 'fetchDiagnoses').and.callThrough();
        component.fetchDiagnoses();
        expect(component.fetchDiagnoses).toHaveBeenCalled();
    });

    it('should test receiveDiagnoses method', () => {
        spyOn(component, 'receiveDiagnoses').and.callThrough();
        component.receiveDiagnoses({
            id: '1283',
            results: [{ id: 1 }],
        });
        expect(component.receiveDiagnoses).toHaveBeenCalled();
    });

    it('should test submitDiagnosis', () => {
        const event = {
            description: 'Test',
            diagnosis: {
                id: '1371',
        ...
        spyOn(component, 'submitDiagnosis').and.callThrough();

        component.getFormOptions({ resetModel: () => {} });
        component.submitDiagnosis(event);
        expect(component.submitDiagnosis).toHaveBeenCalledWith(event);
        component.submitDiagnosis(event2);
        expect(component.submitDiagnosis).toHaveBeenCalledWith(event2);
        component.submitDiagnosis(event3);
        expect(component.submitDiagnosis).toHaveBeenCalledWith(event3);
    });
...
describe('TreatmentPlanComponent fails', () => {
    let component: TreatmentPlanComponent;
    let fixture: ComponentFixture<TreatmentPlanComponent>;
